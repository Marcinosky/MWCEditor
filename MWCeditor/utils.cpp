...
void CommandLineConsole(std::wstring& arg)
{
	...
	// Log
	dbglog = new DebugOutput(L"MWCEditorLog.txt");
...
HRESULT FindAndCreateAppFolder()
{
	...
	AppendPath(DocumentFolder, L"MyWinterCar");
	AppFolder = DocumentFolder;
	AppendPath(AppFolder, L"MWCEditor");
...
bool SaveSettings(const std::wstring &savefilename)
{
	...
	setting += L'\"' + settings[3] + L"\" \"" + std::to_wstring(bAllowScale == 1) + L"\"\n";
	setting += L'\"' + settings[4] + L"\" \"" + std::to_wstring(bEulerAngles == 1) + L"\"\n";
	setting += L'\"' + settings[5] + L"\" \"" + std::to_wstring(bDisplayRawNames == 1) + L"\"\n";
